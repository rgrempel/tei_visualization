...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml" xmlns:str="http://exslt.org/strings"
    ...
    <xsl:key name="namesByType" match="tei:name" use="@type" />

    <xsl:template match="/">
        <pd:names>
            <xsl:for-each select="//tei:name[generate-id(.) = generate-id(key('namesByType', @type))]">
                <pd:type id="{@type}" text="{@type}" type="{@type}" loaded="true">
                <!-- I want to sort by a computed value, which would be simple if I had functions, but I don't.
                    ...
                    <xsl:variable name="names">
                        <xsl:apply-templates select="key('namesByType', @type)[generate-id(.) = generate-id(key('namesByKey', @key))]" />
                    </xsl:variable>
    ...
    <xsl:template match="tei:name">
        <pd:name id="{@key}" key="{@key}" type="{@type}" leaf="true">
